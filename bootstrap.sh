...
awk 'NR>1 {print $NF}' /etc/hosts | grep -Ev 'master|dsnode' > /local/host_list
# # docker
...
echo "export PATH=$PATH:/usr/local/spark/bin" | sudo tee -a ~/.bashrc;
...
# awk 'NR>1 {print $NF}' /etc/hosts | grep -Ev 'master|dsnode' > $HOST_LIST_PATH
